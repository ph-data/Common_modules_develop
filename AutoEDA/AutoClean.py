...
class MissingValues:
    ...
    def handle(self, df, _n_neighbors=3):
        ...
        if self.count_missing != 0:
            ...
            df = df.dropna(how='all')   ### 전체가 다 na인 것은 지우기
                ...
                try:
                    ...
                    if len(test_df.index) != 0:
                        ...
                        if (y == train_df[feature]).all():
                            ...
                            pred = np.exp(model.predict(test_df)) # predict values (로그값 원상복귀)
...
class EncodeCateg:
 ...
 class AutoClean(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, missing_num='auto', missing_categ='auto', encode_categ=['auto'], extract_datetime='s', outliers='winz', outlier_param=1.5, logfile=True, verbose=False):  
        '''
